...
from sklearn.preprocessing import StandardScaler  # 新增：局部标准化
...
def train_one_epoch(model3, train_loader, graph_loader, criterion, optimizer, device, scaler, clip_norm=1.0):
    ...
    for (features1, features3, labels), graph_batch in zip(train_loader, graph_loader):
        ...
        with torch.cuda.amp.autocast():
            logits = model3(features1, graph_batch, features3)
...
def main_worker(args):
    ...
    x1_all, _ = Bert_out(args.dataset)
    x3_all, _ = Bio_feature_out(args.dataset)
    labels_all = DC_labels_tensor.numpy().astype(np.int64)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(x1_all, labels_all)):
        ...
        # 【修复】：在折内拟合标准化器，防止数据泄露
        N_tr1, L1, D1 = tr_x1.shape
        scaler_x1 = StandardScaler()
        tr_x1 = scaler_x1.fit_transform(tr_x1.reshape(-1, D1)).reshape(N_tr1, L1, D1)
        va_x1 = scaler_x1.transform(va_x1.reshape(-1, D1)).reshape(va_x1.shape[0], L1, D1)

        N_tr3, L3, D3 = tr_x3.shape
        scaler_x3 = StandardScaler()
        tr_x3 = scaler_x3.fit_transform(tr_x3.reshape(-1, D3)).reshape(N_tr3, L3, D3)
        va_x3 = scaler_x3.transform(va_x3.reshape(-1, D3)).reshape(va_x3.shape[0], L3, D3)
        ...
        for epoch in range(args.epoch):
            ...
            print(
                f'[Val  ] BalAcc@best={bal_at_thr:.4f} thrBAL={thr_bal:.3f} | lr={optimizer.param_groups[0]["lr"]:.6f}')
            ...
            if metric_for_selection > best_mcc_fold:
                ...
                # 【修复】：同时落盘保存当前最佳折的 Scaler 供测试阶段使用
                joblib.dump(scaler_x1, "best_scaler_x1.pkl")
                joblib.dump(scaler_x3, "best_scaler_x3.pkl")
                ...
                if counter >= patience:
                    print(
                        f"[Early Stop] epoch={epoch + 1}, best BalAcc={best_mcc_fold:.4f} (epoch={best_epoch_fold}, thr={best_thr_fold:.3f})")
                    break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--thr_steps', type=int, default=101)
